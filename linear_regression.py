# -*- coding: utf-8 -*-
"""
Created on Fri Jul 23 07:59:58 2021
...
class linear_regression():
    def __init__(self,x,y):
        ...
        #x是属性对应数据，y是连续值标签
        #x行为一个样本，列为一个属性，需要在最后一列加上一列1
        self.x=np.asarray(x,dtype=float)
        self.y=np.asarray(y,dtype=float)
        
        
        #计算行列式时用到，记录化为上三角前将对角元素化为非0元素时交换的行与列
        self.change_row=0
    ...
    #通过x与y计算权重加上偏置的矩阵w    
    def fit(self):
        t=self.calculate_inversion((np.matmul((self.x).T,self.x)))
        t=np.matmul(t,self.x.T)
        self.w=np.matmul(t,self.y)
    
    
    
    #预测    
    def predict(self,x):
        return np.matmul(x.T,self.w)
